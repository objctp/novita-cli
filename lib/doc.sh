...
# command files into a queryable surface for `nv doc`. Only user-facing commands
# are documented (never library internals like nv::http): each command's
...
nv::doc_intro_summary() {
  nv::doc_intro "$1" | awk 'NF {print; exit}'
}
...
# to its own sub-case rather than by a case arm. Groups are emitted where the
# guard appears, so the order this prints is the order the doc section must
# follow.
nv::doc_verbs() {
