// scripts/build.mjs — prepare the novita-cli npm package for publishing.
//
// nv is a plain Bash CLI, so "building" is light: stamp the version into
...
const FILES = ["bin/nv", "lib", "commands", "LICENSE"];
...
function verifyFiles() {
	...
	for (const f of FILES) {
		...
		if (f === "bin/nv") {
			...
			// umask (or a checkout that dropped the bit) can't break `npx nv`.
			const mode = statSync(p).mode;
			if (!(mode & 0o111)) {
				process.stdout.write(`build: bin/nv not executable; chmod +x\n`);
				chmodSync(p, 0o755);
			}
		}
	}
...
function main() {
	...
	process.stdout.write(`build: ready to publish novita-cli@${version}\n`);
}
