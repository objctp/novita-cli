#!/usr/bin/env bash
# Generate a structured markdown manual from `nv doc` output.
#
# Every user-facing command/verb already carries a reference block in its
# source comments, rendered by `nv doc`. This script reuses that surface so the
...
#   docs/<command>-<group>-<subverb>.md   for group verbs
...
#
# Usage: scripts/gen-manual.sh [command ...]
set -euo pipefail
cd "$(dirname "$0")/.."
ROOT="$PWD"
OUT="$ROOT/docs"
...
nvdoc() { "$ROOT/bin/nv" doc "$@" 2>/dev/null; }
...
# Transform `nv doc` text (stdin) into markdown. $1 = heading level for the
# title line (`nv …`). The usage line is emitted as a fenced block directly
...
transform() {
  awk -v hl="$1" '
    ...
    NR==1 && $0 ~ /^nv / {
      base = substr($0, 4)                 # strip leading "nv "
      print hashes(hl) " " $0
      next
    }
...
# Collect verb names from a command-level `nv doc` dump.
verbs_of() { awk '/^Verbs:/{f=1;next} f&&/^  [a-z]/{print $1}' <<<"$1"; }

# True when $1 (a `nv doc <verb>` dump) is a group verb (owns a Verbs: list).
...
# docs/<name>-<verb>.md. A group verb (one owning sub-verbs) gets its own
# category page docs/<name>-<verb>.md plus a page per sub-verb
# docs/<name>-<verb>-<subverb>.md.
gen_command() {
  ...
  for v in $verbs; do
    ...
    if is_group "$vt"; then
      ...
      subs="$(verbs_of "$vt")"
      for s in $subs; do
        { transform 1 <<<"$(nvdoc "$name" "$v" "$s")"; } | normalize >"$OUT/$name-$v-$s.md"
        printf 'docs/%s.md\n' "$name-$v-$s"
      done
    else
      { transform 1 <<<"$vt"; } | normalize >"$OUT/$name-$v.md"
...
gen_index() {
  {
    echo "# Novita CLI (nv)"
    echo
    echo "Reference for the Novita CLI (\`nv\`). Each command has its own page:"
    echo
    nvdoc | while read -r line; do
      # lines look like: "nv pod          GPU instances ..."
      [[ "$line" =~ ^nv\ ([a-z-]+)[[:space:]]+(.*)$ ]] || continue
...
# index is always refreshed (one `nv doc` call) since any command's summary may
# have changed.
main() {
  local -a names=("$@")
  if [ ${#names[@]} -eq 0 ]; then
